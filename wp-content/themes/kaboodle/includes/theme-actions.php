...
if (!function_exists('woo_custom_styling')) {
	function woo_custom_styling() {
		...
		if ($hover)
			$output .= 'a:hover, .post-meta a:hover, .post-more a:hover, #latest-blog-posts .post h2.title a:hover, .widget ul li a:hover, #tabs .inside a:hover, #comments .comment-head a:hover, .dribbbles strong {color:'.$hover.'}' . "\n";
		...
		<script type="text/javascript">
			...
			jQuery("#loopedSlider").loopedSlider({
				<?php
					$autoStart = 0;
					$slidespeed = 600;
					if ( get_option("woo_single_auto") == "true" ) 
		   				$autoStart = get_option("woo_single_interval");
					else 
		   				$autoStart = 0;
					if ( get_option("woo_single_speed") <> "" ) 
						$slidespeed = get_option("woo_single_speed");
				?>
