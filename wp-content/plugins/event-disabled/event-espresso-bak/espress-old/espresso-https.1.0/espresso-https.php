<?php
/*
 Plugin Name: Event Espresso - SSL/HTTPS
 Plugin URI: http://eventespresso.com
 Description: Event Espresso HTTPS is intended to be an all-in-one solution to using SSL on your Event Espresso sites.
...
/**
 * Class for the WordPress plugin Espresso HTTPS
 *
 * @author Event Espresso
 * @original author Mike Ems
 * @package EspressoHTTPS
 * @copyright Copyright 2011
 *
 * @return object
 *
 */
if ( !class_exists('EspressoHTTPS') ) {
	class EspressoHTTPS {
		...
		var $options_default = array(
			'espresso-https_internalurls' => 1,		// Force internal URL's to HTTPS
			'espresso-https_externalurls' => 0,		// Force external URL's to HTTPS
			'espresso-https_bypass' => 0,				// Bypass option to check if external elements can be loaded via HTTPS
			'espresso-https_disable_autohttps'=> 0,	// Prevents WordPress 3.0+ from making all links HTTPS when viewing a secure page.
			'espresso-https_exclusive_https'=> 0,		// Exclusively force SSL on posts and pages with the `Force SSL` option checked.
			'espresso-https_frontpage'=> 0,			// Force SSL on front page
			'espresso-https_sharedssl'=> 0,			// Enable Shared SSL
			'espresso-https_sharedssl_admin' => 0,		// Shared SSL for admin panel
			'espresso-https_sharedssl_host' => '',		// Hostname for Shared SSL
			'espresso-https_external_urls' => array()	// External URL's that are okay to rewrite to HTTPS
		);
		...
		function __construct() {
			...
			// Shared SSL
			if ( get_option('espresso-https_sharedssl') == 1 && get_option('espresso-https_sharedssl_host') != '' ) {
				// Turn on Shared SSL
				$this->shared_ssl = true;
				// Assign HTTPS URL to Shared SSL Host
				$this->https_url = get_option('espresso-https_sharedssl_host');
			...
			if ( $GLOBALS['pagenow'] == 'wp-login.php' && $this->shared_ssl && !$this->is_ssl() && get_option('espresso-https_sharedssl_admin') == 1 ) {
				$this->redirect('https');
			}
			...
			if ( is_admin() ) {
				...
				// Add admin menus
				if ( version_compare( get_option('events_detail_tbl_version'), '3.2.P', '<' ) == true ) {
				...
				if ( @$_GET['page'] == 'espresso_https' ) {
					wp_enqueue_style('espresso-https', $this->plugin_url . '/css/admin.css', $this->plugin_version, true);
			...
			if ( get_option('espresso-https_disable_autohttps') == 1 && !is_admin() && strpos(get_option('home'), 'https://') === false ) {
		...
		function process($buffer) {
			if ( $this->is_ssl() ) {
				...
				$external_urls = get_option('espresso-https_external_urls');
				...
				for ($i = 0; $i<=sizeof($matches[0]); $i++) {
					...
					if ( $type == 'img' || $type == 'script' || $type == 'embed' ||
						...
						if ( is_admin() && $type == 'img' ) {
							...
							if ( strpos($url, $this->http_url) !== false && get_option('espresso-https_internalurls') == 1 ) {
								$buffer = str_replace($html, str_replace($url, $this->replace_http_url($url), $html), $buffer);
							} else if ( strpos($url, $this->replace_http($this->http_url)) !== false && $this->shared_ssl ) {
								$buffer = str_replace($html, str_replace($url, $this->replace_http_url($url), $html), $buffer);
							} else if ( $this->shared_ssl && get_option('espresso-https_internalurls') == 1 && strpos($html, $this->http_url) !== false ) {
								$buffer = str_replace($html, str_replace($url, $this->replace_http_url($url), $html), $buffer);
							} else if ( strpos($url, $this->https_url) === false && strpos($url, 'https://') === false && get_option('espresso-https_externalurls') == 1 ) {
								if ( get_option('espresso-https_bypass') == 1 ) {
									...
									if ( !in_array($url, $external_urls) ) {
										$external_urls[] = $url;
										update_option('espresso-https_external_urls', $external_urls);
									}
								}
							}
						}
			...
			for ($i = 0; $i<=sizeof($matches[0]); $i++) {
				...
				if ( preg_match("/page_id=([\d]+)/", parse_url($url, PHP_URL_QUERY), $postID) == 1 ) {
					...
					if ( get_option('show_on_front') == 'posts' ) {
						$post = true;
						$force_ssl = (( get_option('espresso-https_frontpage') == 1 ) ? true : false);
					} else {
				...
				if ( $post ) {
					...
					if ( $force_ssl ) {
						$buffer = str_replace($html, str_replace($url, $this->replace_http_url($url), $html), $buffer);
					} else if ( get_option('espresso-https_exclusive_https') == 1 ) {
			...
			if ( $this->shared_ssl && get_option('espresso-https_internalurls') == 1 ) {
		...
		function check_https() {
			...
			if ( is_front_page() && get_option('show_on_front') == 'posts' ) {
				if ( get_option('espresso-https_frontpage') == 1 && !$this->is_ssl() ) {
					$this->redirect('https');
				} else if ( get_option('espresso-https_frontpage') != 1 && get_option('espresso-https_exclusive_https') == 1 && $this->is_ssl() ) {
					$this->redirect('http');
				}
				...
				if ( !$this->is_ssl() && $forceSSL ) {
					$this->redirect('https');
				} else if ( get_option('espresso-https_exclusive_https') == 1 && !$forceSSL ) {
					$this->redirect('http');
				}
			}
		}
		...
		function post_checkbox() {
			...
			wp_nonce_field(plugin_basename(__FILE__), 'espresso-https');

			$checked = false;
			if ( $post->ID ) {
				$checked = get_post_meta($post->ID, 'force_ssl', true);
			}
			echo '<div class="misc-pub-section misc-pub-section-last" style="border-top: 1px solid #EEE;"><label>'.__('Force SSL:', 'event_espresso').' <input type="checkbox" value="1" name="force_ssl" id="force_ssl"'.(($checked) ? ' checked="checked"' : '').' /></label></div>';
		}
		...
		function post_save( $post_id ) {
			if ( array_key_exists('espresso-https', $_POST) ) {
				if ( !wp_verify_nonce($_POST['espresso-https'], plugin_basename(__FILE__))) {
					return $post_id;
				}
		 ...
		 */
		function menu() {
			add_options_page('Espresso HTTPS Settings', 'Espresso HTTPS', 'manage_options', 'espresso_https', array(&$this, 'settings'));
		}
		...
		function plugin_links($links, $file) {
			...
			$links[] = '<a href="admin.php?page=espresso_https" title="Espresso HTTPS Settings">Settings</a>';
			return $links;
		}
		...
		function buffer_start() {
			if ( get_option('espresso-https_externalurls') == 1 && get_option('espresso-https_bypass') != 1 ) {
			...
			if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
				...
				foreach ($this->options_default as $key => $default) {
					if ( !array_key_exists($key, $_POST) && $default == 0 ) {
						...
						} else if ( $key == 'espresso-https_externalurls' && @ini_get('allow_url_fopen') != 1 ) {
							$errors[] = '<strong>External HTTPS Elements</strong> - PHP configuration error: allow_url_fopen must be enabled.';
							$_POST[$key] = 0;
						} else if ( $key == 'espresso-https_disable_autohttps' && version_compare(get_bloginfo('version'), '3.0', '<') ) {
							$_POST[$key] = 0;
						}
...
<div class="wrap">
		...
		<h2><?php _e('Event Espresso HTTPS Settings', 'event_espresso'); ?></h2>
...
?>
<?php //do_action('espresso_right_column');?>

	<div id="eehttps-main">
		<div id="post-body">
        
			<form name="form" id="espresso-https" action="admin.php?page=espresso_https" method="post">
			<?php settings_fields('espresso-https'); ?>

			<fieldset>
				<label for="espresso-https_internalurls"><input name="espresso-https_internalurls" type="checkbox" id="espresso-https_internalurls" value="1"<?php echo ((get_option('espresso-https_internalurls')) ? ' checked="checked"' : ''); ?> /> <strong>Internal HTTPS Elements</strong></label>
				<p><?php _e('Force internal elements to HTTPS when viewing a secure page.', 'event_espresso'); ?></p>
				<p class="description"><?php _e('Fixes most partially encrypted errors.', 'event_espresso'); ?></p>
			</fieldset>

			<fieldset>
				<label for="espresso-https_externalurls"><input name="espresso-https_externalurls" type="checkbox" id="espresso-https_externalurls" value="1"<?php echo ((get_option('espresso-https_externalurls')) ? ' checked="checked"' : ''); ?> /> <strong><?php _e('External HTTPS Elements', 'event_espresso'); ?></strong></label>
				<p><?php _e('Attempt to automatically force external elements to HTTPS when viewing a secure page. External elements are any element not hosted on your domain.', 'event_espresso'); ?></p>
				<p class="description"><?php _e('Warning: This option checks that the external element can be loaded via HTTPS while the page is loading. Depending on the amount of external elements, this could affect the load times of your pages.', 'event_espresso'); ?></p>
			</fieldset>

			<fieldset>
				<label for="espresso-https_bypass"><input name="espresso-https_bypass" type="checkbox" id="espresso-https_bypass" value="1"<?php echo ((get_option('espresso-https_bypass')) ? ' checked="checked"' : ''); ?> /> <strong>Bypass External Check</strong></label>
				<p><?php _e('Disable the option to check if an external element can be loaded over HTTPS.', 'event_espresso'); ?></p>
				<p class="description"><?php _e('Warning: Bypassing the HTTPS check for external elements may cause elements to not load at all. Only enable this option if you know that all external elements can be loaded over HTTPS.', 'event_espresso'); ?></p>
			</fieldset>

<?php if ( version_compare(get_bloginfo('version'), '3.0', '>=') ) { ?>
			<fieldset>
				<label for="espresso-https_disable_autohttps"><input name="espresso-https_disable_autohttps" type="checkbox" id="espresso-https_disable_autohttps" value="1"<?php echo ((get_option('espresso-https_disable_autohttps')) ? ' checked="checked"' : ''); ?> /> <strong>Disable Automatic HTTPS</strong></label>
				<p><?php _e('Prevents WordPress 3.0+ from making all links HTTPS when viewing a secure page.', 'event_espresso'); ?></p>
				<p class="description"><?php _e('When a page is viewed via HTTPS in WordPress 3.0+, all internal page, category and post links are forced to HTTPS. This option will disable that.', 'event_espresso'); ?></p>
			</fieldset>

<?php } ?>
			<fieldset>
				<label for="espresso-https_exclusive_https"><input name="espresso-https_exclusive_https" type="checkbox" id="espresso-https_exclusive_https" value="1"<?php echo ((get_option('espresso-https_exclusive_https')) ? ' checked="checked"' : ''); ?> /> <strong><?php _e('Force SSL Exclusively', 'event_espresso'); ?></strong></label>
				<p><?php _e('Exclusively force SSL on posts and pages with the \'Force SSL\' option checked. All others are redirected to HTTP.', 'event_espresso'); ?></p>
				<p class="description"><?php _e('Espresso HTTPS adds a \'Force SSL\' checkbox to each post and page right above the publish button', 'event_espresso'); ?> (<a href="<?php echo $this->plugin_url; ?>/screenshot-2.png" class="thickbox"><?php _e('screenshot', 'event_espresso'); ?></a>). <?php _e('When selected, the post or page will be forced to HTTPS. With this option enabled, all posts and pages without \'Force SSL\' checked will be redirected to HTTP.', 'event_espresso'); ?></p>
			</fieldset>

			<fieldset>
				<label for="espresso-https_sharedssl"><input name="espresso-https_sharedssl" type="checkbox" id="espresso-https_sharedssl" value="1"<?php echo ((get_option('espresso-https_sharedssl')) ? ' checked="checked"' : ''); ?> /> <strong>Shared SSL</strong></label>
				<p><?php _e('Enable this option if you are using a Shared SSL certificate and your Shared SSL Host is something other than', 'event_espresso'); ?> '<?php echo $this->replace_http($this->http_url); ?>/'.</p>
				<label><strong><?php _e('Shared SSL Host', 'event_espresso'); ?></strong> <input name="espresso-https_sharedssl_host" type="text" id="espresso-https_sharedssl_host" value="<?php echo get_option('espresso-https_sharedssl_host'); ?>" /></label>
			</fieldset>

			<fieldset>
				<label for="espresso-https_sharedssl_admin"><input name="espresso-https_sharedssl_admin" type="checkbox" id="espresso-https_sharedssl_admin" value="1"<?php echo ((get_option('espresso-https_sharedssl_admin')) ? ' checked="checked"' : ''); ?> /> <strong>Force Shared SSL Admin</strong></label>
				<p><?php _e('Enable this option if you are using a Shared SSL certificate and you only want to access your admin panel over HTTPS.', 'event_espresso'); ?></p>
				<p class="description"><?php _e('Notice: FORCE_SSL_ADMIN and FORCE_SSL_LOGIN can not be set to true in your wp-config.php.', 'event_espresso'); ?></p>
			</fieldset>

<?php if ( get_option('show_on_front') == 'posts' ) { ?>
			<fieldset>
				<label for="espresso-https_frontpage"><input name="espresso-https_frontpage" type="checkbox" id="espresso-https_frontpage" value="1"<?php echo ((get_option('espresso-https_frontpage')) ? ' checked="checked"' : ''); ?> /> <strong>HTTPS Front Page</strong></label>
				<p><?php _e('It appears you are using your latest posts for your home page. If you would like that page to have SSL enforced, enable this option.', 'event_espresso'); ?></p>
			</fieldset>
...
<?php
	...
	} // End EspressoHTTPS Class
}

if ( class_exists('EspressoHTTPS') ) {
	$espresso_https = new EspressoHTTPS();
	register_activation_hook( __FILE__, array(&$espresso_https, 'install'));
}

function espresso_https_mnu(){
	$espresso_https = new EspressoHTTPS();
	$espresso_https->settings();
}

// Use Espresso HTTPS wp_set_auth_cookie method for WordPress' wp_set_auth_cookie pluggable function if using Shared SSL
...
// Use Espresso HTTPS auth_redirect method for WordPress' auth_redirect pluggable function if using Shared SSL
