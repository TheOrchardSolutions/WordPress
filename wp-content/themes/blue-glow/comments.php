...
<?php if ($comments) : ?>
	...
	<?php foreach (array_reverse($comments) as $comment) : ?>
		...
		<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
			<p style="color: #FFF;"><?php comment_author_link() ?> wrote:</p>
			<?php if ($comment->comment_approved == '0') : ?>
			<p><em>Your comment is awaiting moderation.</em></p>
			<?php endif; ?>

			<?php comment_text() ?>
			<p style="margin-left: 48%; color: #FFF;">. . .</p>
		</li>
...
<p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" /> <label for="email"><small>Email (will not be published) <?php if ($req) echo "(required)"; ?></small></label></p>
